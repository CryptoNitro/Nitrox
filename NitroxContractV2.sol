...
contract NitroxContractV2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 playToEarnFee, uint8 buybackFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <=maxTaxFee,"TaxFee error");
        require(liquidityFee >= 0 && liquidityFee <=maxLiqFee,"LiquidityFee error");
        require(burnFee >= 0 && burnFee <=maxBurnFee,"BurnFee error");
        require(walletFee >= 0 && walletFee <=maxWalletFee,"WalletFee error");
        require(playToEarnFee >= 0 && playToEarnFee <=maxPlayToEarnFee,"PlayToEarnFee error");
        require(buybackFee >= 0 && buybackFee <=maxBuybackFee,"BuyBackFee error");
        _taxFee = taxFee;
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent >= minMxTxPercentage && maxTxPercent <=100,"Error");
    ...
    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner() {
        require(maxWalletPercent >= minMxWalletPercentage && maxWalletPercent <=100,"Error");
    ...
    function setPlayToEarnWallet(address payable newPlayToEarnWallet) external onlyOwner {
        require(newPlayToEarnWallet != address(0), "ZERO ADDRESS");
